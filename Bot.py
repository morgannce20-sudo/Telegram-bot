...
Niveau de confiance: XX% ⭐⭐⭐"""
        ...
        bot.send_message(CHAT_ID, f"Erreur pronostic auto: {str(e)}")
...
def start(message):
    bot.reply_to(message, "Bonjour! Je suis ton expert en paris sportifs! 🏆\n\nDemande-moi un pronostic ou attends mes analyses automatiques 2h avant chaque match!")
...
def repondre(message):
    try:
        ...
        prompt = f"""Tu es un expert en paris sportifs en 2025.
Mbappé a quitte le PSG en 2024 pour le Real Madrid.
...
Format de reponse pour : {message.text}
